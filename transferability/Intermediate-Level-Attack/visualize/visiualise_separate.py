...
def plot_histogram_name(x, name, picname, folder):
    ...
    _ = plt.hist(newvalues, bins = 8, color = 'Blue', alpha = 0.5)
    '''
    figname = 'IMAGES/'+ folder + '/hist/' + picname + name + '.png'     
    _ = plt.xlabel('Intensity Value', fontsize=18)
    _ = plt.ylabel('Count', fontsize=18)
    #_ = plt.legend(['Total', 'Red_Channel', 'Green_Channel', 'Blue_Channel'])
    plt.margins(0,0)
...
def plotting_name_sep(x, x_adv, x_grad, epsilon, name, picname, folder):
    ...
    if name == 'FW':
       ...
       plt.title( 'PGDnucl' + ' Perturbed', fontsize=20)
    elif name == 'FW_group':
       plt.title('Group-FWnucl' + ' Perturbed', fontsize=20)
    else:
       plt.title(name.upper() + ' Perturbed', fontsize=20)
    #plt.show()
    figname =  'IMAGES/'+ folder + '/' + picname + name + '_1.png' 
    ...
    #plt.subplot(111)
    ax = plt.gca()
    im = ax.imshow(np.sum(np.abs(x_grad), axis=2), cmap='hot', interpolation='nearest')
    plt.axis('off')
    if name == 'FW':
       plt.title('FWnucl' + ' Perturbation', fontsize=20)
    elif name == 'FW_group':
       plt.title('Group-FWnucl' + ' Perturbation', fontsize=20)
    elif name == 'pgdnucl':
       plt.title('PGDnucl' + ' Perturbation', fontsize=20)
    else:
       plt.title(name.upper() + ' Perturbation', fontsize=20) 
    im.set_clim(vmin=0, vmax=0.06)
    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    plt.colorbar(im, cax=cax)
    figname =  'IMAGES/'+ folder + '/' + picname + name + '_2.png' 
    plt.savefig(figname, bbox_inches='tight',pad_inches = 0.1)
    ...
    if name == 'FW':
       ...
       grid[0].set_title('Group-FWnucl' + ' Perturbed', fontsize=20)
    else:
       grid[0].set_title(name.upper() + ' Perturbed', fontsize=20)
    plt.margins(0,0)
    plt.tight_layout(h_pad=0) 
    ...
    imc = grid[0].imshow(np.sum(np.abs(x_grad), axis=2), cmap='hot', interpolation='nearest')
    grid[0].axis('off')


    if name == 'pgdnucl':
       grid[0].set_title('PGDnucl' + ' Perturbation', fontsize=20)

    if name == 'FW':
       grid[0].set_title('FWnucl' + ' Perturbation', fontsize=20)
    elif name == 'FW_group':
       grid[0].set_title('Group-FWnucl' + ' Perturbation', fontsize=20)
    elif name == 'pgdnucl':
       grid[0].set_title('PGDnucl' + ' Perturbation', fontsize=20)
    else:
       grid[0].set_title(name.upper() + ' Perturbation', fontsize=20) 
...
def gen_adv_ex_FW(img_variable):
   ...
   FW = FW_vanilla_batch(radius=eps, normalizer=normalize, T_max=niters, rand=False, device='cuda')
...
def gen_adv_ex_FW_group(img_variable):
   ...
   eps = 3
   FW = FW_vanilla_batch(radius=eps, normalizer=normalize, T_max=niters, device='cuda', group_norm='group', nbr_par=50, rand=False)
   x_adversarial = FW.attack(model, img_variable, target, niters)
   x_grad = x_adversarial - img_variable
...
picnames = ['pics/ILSVRC2012_val_00038256.JPEG']
...
labelnames = ['acorn', 'otterhound', 'marmot', 'jaguar', 'bee-eater', 'sulphur-crested-cockatoo', 'quail', 'lion'] #picnames_labels_eps1
picnames_eps3 = ['pics/ILSVRC2012_val_00009649.JPEG', 'pics/ILSVRC2012_val_00023805.JPEG'] #picnames_eps3
picnames_labels_eps3 = ['bee-eater', 'sulphur-crested-cockatoo'] #picnames_labels_eps3
picnames_eps5 = ['pics/ILSVRC2012_val_00033162.JPEG', 'pics/ILSVRC2012_val_00024234.JPEG'] #picnames_eps5
picnames_labels_eps5 = ['quail', 'lion'] #picnames_labels_eps5

folder = 'pic_sep'
logname = 'IMAGES/'+ folder + '/results.txt'
...
#picnames = ['new_pics/ILSVRC2012_val_00002255.JPEG'] #for different networks
...
#for filename in picnames_eps5:
   ...
   for name, method in models:
      ...
      plotting_name_sep(img_variable.detach(), x_adversarial.detach(), x_grad.detach(), eps, name, dictImagesNames[filename], folder)
      #images_cum += img_variable.cpu()
      #adv_cum += x_adversarial.cpu()
      #plot_histogram(x_grad, name, filename)
      plot_histogram_name(x_grad, name, dictImagesNames[filename], folder)
