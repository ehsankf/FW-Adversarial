...
def test( model, device, example, epsilon, mode):
    model.eval()
    # Accuracy counter
    correct = 0
    adv_examples = []
    ...
    if mode == 'FW':
       ...
       FW = FW_vanilla_batch(model, loss,
                              type_ball='nuclear',
                              p=1,
                              mask=False,_, final_predpgdnucl = torch.max(outputpgdnucl.data, 1)
                              radius=epsilon,
                              stepsize=1,
                              eps=1e-10,
                              T_max=20,
...
params.append({'mode': 'FW', 'epsilon': 1.0})
params.append({'mode': 'FW', 'epsilon': 3.0})
params.append({'mode': 'FW', 'epsilon': 5.0})
...
plt.show()
'''
names = {'Orig', 'FWnucl', 'PGDNucl', 'PGD', 'FGSM'}
...
plt.savefig('mnist_lenet_compared.png', bbox_inches='tight',pad_inches = 0)
plt.show()
