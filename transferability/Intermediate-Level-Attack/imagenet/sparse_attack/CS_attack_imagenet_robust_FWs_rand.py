...
sys.path.insert(0, '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/StructuredAttack/sparse_imperceivable_attacks')
...
def sampler(dataset, nb_classes, nb_examples):
    indicies = np.empty((nb_classes, 0)).tolist()
    print("arr len: ", len(indicies))
    for idx, (_, y) in enumerate(test_loader):
        indicies[y.item()].append(idx)
    samples = []
    pdb.set_trace()
    for cl_idx in indicies:
        np.random.shuffle(cl_idx)
        samples.extend(cl_idx[:nb_examples])
    return samples
...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            with open(logname, 'a') as logfile:
               logwriter = csv.writer(logfile, delimiter=',')
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, eval_x_adv_linf/succ_tot, eval_x_adv_l2/succ_tot, eval_x_adv_l0/succ_tot, eval_x_adv_lnuc/succ_tot, correct_pgd, total])
...
def main():
    ...
    if adv_atc == 'CS': 
      for eps in [-1.]: 
        pgd_params = {'attack': 'CS', 'epsilon': eps, 'num_steps': 10, 'step_size': 0.1,
    ...
    if adv_atc == 'FW_group':
      for eps in [1., 3.0, 5.0]: 
        # eval_test(model, device, test_loader)
        #eval_adv_test_whitebox(model, device, test_loader)
        #eval_adv_test_Carlini(model, device, test_loader)
        #eval_test(model, device, test_loader)
        step_size =  1
        pgd_params = {'attack': 'FW_group', 'epsilon': eps, 'num_steps': 10, 'step_size': step_size,
...
if __name__ == '__main__':
        ...
        torch.utils.data.Subset(datasets.ImageFolder(valdir, transforms.Compose([
