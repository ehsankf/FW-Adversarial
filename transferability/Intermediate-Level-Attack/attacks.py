...
class FW_vanilla_batch():
    ...
    def attack(self, model, images: torch.Tensor,
               labels: torch.Tensor, T_max, visualize=False, display=4) -> torch.Tensor:
        '''
        Method:
        -------
        ...
        maxpixelval = 1#torch.max(images.data)
        minpixelval = 0#torch.min(images.data)
        model.eval()
        while iter < T_max:
            ...
            x_adv = torch.clamp(x_adv, minpixelval, maxpixelval)
...
def wrap_attack_FW(attack, params, dataset='cifar10'):
    ...
    def wrap_f(model, images, labels, niters, use_Inc_model=False):
        # attack should process by batch
        return FW.attack(model, images, labels, niters)
