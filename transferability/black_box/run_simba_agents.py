...
'''
python run_simba.py --num_iters 5 --pixel_attack  --freq_dims 224 --num_runs 10 --data_root ../Intermediate-Level-Attack/imagenet  --batch_size 1
'''
...
batchfile = '%s/agents_images_%s_%d.pth' % (args.sampled_image_dir, args.model, args.num_runs)
...
for i in range(N):
    ...
    if args.targeted:
        prefix += '_targeted'    
    savefile = '%s/%s_%s_to_%s_%d_%d_%d_%.4f_%s%s.pth' % (
