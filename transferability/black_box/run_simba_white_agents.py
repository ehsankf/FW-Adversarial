...
'''
python run_simba_white_agents.py --num_iters 10000 --pixel_attack  --freq_dims 224 --num_runs 100 --data_root ../Intermediate-Level-Attack/imagenet  --batch_size 1 --model_src googlenet --epsilon 0.02
...
def fgsm(model_src, model, 
    ...
    with torch.enable_grad():
        #loss = F.cross_entropy(model_src(X.cuda()).cpu(), y) - F.cross_entropy(model_src(X.cuda()).cpu(), y)
...
# runs simba on a batch of images <images_batch> with true labels (for untargeted attack) or target labels
# (for targeted attack) <labels_batch>
def dct_attack_batch_fdm(model_src, model, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, order='rand', targeted=False, pixel_attack=False, log_every=1):
    batch_size = images_batch.size(0)
    image_size = images_batch.size(2)
    # sample a random ordering for coordinates independently per batch element
    if order == 'rand':
    ...
    for k in range(max_iters):
        ...
        if targeted:
            #right_improved = right_probs.gt(torch.max(prev_probs[remaining_indices], left_probs)) and right_preds_src.eq(labels_batch[remaining_indices])
            right_improved = right_preds_src.eq(labels_batch[remaining_indices])
        else:
            #right_improved = right_probs.lt(torch.min(prev_probs[remaining_indices], left_probs)) and right_preds_src.eq(labels_batch[remaining_indices])
            right_improved = right_preds_src.eq(labels_batch[remaining_indices])
        probs_k = prev_probs.clone()
...
batchfile = '%s/white_agents_images_%s_%d.pth' % (args.sampled_image_dir, args.model, args.num_runs)
...
for i in range(N):
    ...
    if args.targeted:
        prefix += '_targeted'    
    savefile = '%s/white_%s_%s_to_%s_%d_%d_%d_%.4f_%s%s.pth' % (
        args.result_dir, prefix, args.model_src, args.model, args.num_runs, args.num_iters, args.freq_dims, args.epsilon, args.order, args.save_suffix)
