...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            x_adv_norm = x_adv # normalize(x_adv) if not('auto-attack' in attack) else x_adv
            x_norm = x # normalize(x) if not('auto-attack' in attack) else x
...
def mk_log_file(attack, num_steps, epsilon):
        ...
        summary = os.path.join(os.path.dirname(args.model_path), 'summary')
        logger = os.path.join(os.path.dirname(args.model_path), 'logger')
...
def main():
    ...
    if adv_atc == 'auto-attackL2': 
      for eps in [0.5, 1.0]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'auto-attackL2', 'epsilon': eps, 'norm': 'L2', 'num_steps': 20, 'step_size': step_size, 
...
if __name__ == '__main__':
        ...
        for adv_atc in ['FW', 'pgdl2']:
            args.model = 'ResNet18'
            args.model_path = 'net_robust_adversarial_eps3_FW/net_epoch150.ckpt'
            main()    
        """
        for adv_atc in ['FW']:
        # for adv_atc in ['FW', 'pgd', 'pgdl2', 'auto-attack']:
        # for adv_atc in ['auto-attackL2']:
        # for adv_atc in ['FW']:

            args.model = 'ResNet18'
            # args.model_path = 'ResNet_robust_adversarial_eps3_FW/net_epoch70.ckpt' # 'net_robust_adversarial_FW_eps5_iter10p/net_epoch160.ckpt'
            # args.model_path = 'net_robust_adversarial_FW_eps5_iter10/net_epoch170.ckpt'
            # args.model_path = 'net_robust_adversarial_FW/net_epoch190.ckpt' # 'ResNet_robust_adversarial_eps3_FW/net_epoch70.ckpt'
            # args.model_path = 'ResNet_robust_adversarial_eps3_FW/net_epoch70.ckpt'
            args.model_path = 'ResNet_robust_adversarial_eps3_FW/net_epoch70.ckpt'
            main()
