...
import torch
import pdb
from SSIM import SSIM, calculate_psnr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_model', type=str, default='AWP', choices=['AWP', 'TRADES-AWP', 'LBGAT'],
    ...
    epsilons_dict = {'auto_attack': [4./255., 8./255., 16./255., 32./255.], 'FW_attack': [1., 3., 5.]}   
    adv_shape = {'AWP':10000, 'TRADES-AWP': 10000, 'LBGAT': 10000}[args.train_model] 
    keys = ["adv_acc", "l2", "lnuc", "SSIM", "PSNR"]
    print_dict = {k: [] for k in keys}
    for attack in ['FW_attack', 'auto_attack']:
        for epsilon in epsilons_dict[attack]:
           ...
           stat_dict['SSIM'] = metric.compute()
    
           stat_dict['PSNR']  = calculate_psnr(stat_dict['x_test'], stat_dict['adv_complete'])
           for k in keys:
               if k == 'SSIM':
                   print_dict[k].append(f'{stat_dict[k]: .4f}')
               else:
                   print_dict[k].append(f'{stat_dict[k]: .2f}')
           
           print(f'attack: {attack} epsilon: {epsilon} nat_acc: {stat_dict["nat_acc"]: .2f} adv_acc: {stat_dict["adv_acc"]: .2f} linf: {stat_dict["linf"]: .2f} l2: {stat_dict["l2"]: .2f}, l0: {stat_dict["l0"]: .2f}, lnuc: {stat_dict["lnuc"]: .2f}, SSIM: {stat_dict["SSIM"]: .4f}, PSNR:, {stat_dict["PSNR"]: .4f}')
     
    for k in keys:
        print(f'{k} {" &".join(map(str, print_dict[k]))}')
