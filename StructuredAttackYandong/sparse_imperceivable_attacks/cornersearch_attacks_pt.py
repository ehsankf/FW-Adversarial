      ...
      if corr_pred[c]:
        ...
        for counter in range(self.n_corners):
          ...
          if not pred.all() and not found:
            ...
            found = True
            print('Point {} - adversarial example found changing 1 pixel-Linf {}'.format(c, np.amax(np.abs(adv[c] - x_nat[c]))))
             ...
             for c2 in range(self.n_classes):
               if not found:
                 ...
                 if np.sum(pred.astype(np.int32)) < self.n_iter and not found:
                   ...
                   print('Point {} - adversarial example found changing {} pixels-Linf {}'.format(c, np.sum(np.amax(np.abs(adv[c] - x_nat[c]) > 1e-10, axis=-1), axis=(0,1)), np.amax(np.abs(adv[c] - x_nat[c]))))
