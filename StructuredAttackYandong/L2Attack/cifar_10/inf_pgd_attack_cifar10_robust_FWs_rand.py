...
sys.path.insert(0, '../')
from pgd_L2 import PGD as PGDL2
...
def calculate_mse(img1, img2, max_val=1):
    img1 = img1.reshape((img1.shape[0], -1))
    img2 = img2.reshape((img2.shape[0], -1))
    mse = np.mean((img1 - img2) ** 2, axis=1)
    mse = mse.sum(axis=0)
...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
             mask, subsampling, size_groups, logname, summaryname, with_attack=True, norm='Linf'):
        logging.info('eval')
        model.eval()
        eval_loss_clean = []
        eval_acc_clean = []
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Test_Loss', 'Clean_Test_Acc', 'Test_Acc', 'Success_rate', 
                              'Linf', 'L2_norm', 'L0_norm', 'Lnuc_norm', 'Correct', 'Total', 'SSIM', 'PSNR'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            elif 'auto-attack' in attack:
                x_adv = adversary.run_standard_evaluation(x, y,
                                                         bs=x.size(0))
                x_adv.to(device) 
            #x_adv = normalize(x_adv).to(device)   
            #x = normalize(x).to(device)          
            x_adv_norm = x_adv if 'auto-attack' in attack else normalize(x_adv)
            x_norm = x if 'auto-attack' in attack else normalize(x)  
            outputs_pgd = model(x_adv_norm)
            ...
            metric.update([x_adv, x])
            eval_ssim = metric.compute()
            
            eval_mse += calculate_mse(x_norm.detach().cpu().numpy(), x_adv_norm.detach().cpu().numpy())
            eval_psnr = 20 * np.log10( 1 / np.sqrt(eval_mse / total))
            ...
            with open(logname, 'a') as logfile:
               ...
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total,
                                  eval_x_adv_linf/total, eval_x_adv_l2/total, eval_x_adv_l0/total, eval_x_adv_lnuc/total, correct_pgd, total, eval_ssim, eval_psnr])
...
def mk_log_file(attack, num_steps, epsilon):
        ...
        if not os.path.isdir('summaryinf'):
            os.mkdir('summaryinf')

        if not os.path.isdir('loggerinf'):
            os.mkdir('loggerinf')

        log_dir = attack + 'iter' + str(num_steps) + 'eps' + str(epsilon) + args.model + args.model_path.split('/')[1].split('.')[0]
        if not os.path.isdir('loggerinf/log_rand_cross_' + attack + '_' + args.model + '/'):
           os.mkdir('loggerinf/log_rand_cross_' + attack + '_' + args.model + '/')
        if not os.path.isdir('summaryinf/sum_rand_cross_' + attack + '_' + args.model + '/'):
           os.mkdir('summaryinf/sum_rand_cross_' + attack + '_' + args.model + '/')

        logname = ('loggerinf/log_rand_cross_' + attack + '_' + args.model + '/' + log_dir + '.csv')
        summaryname = ('summaryinf/sum_rand_cross_' + attack + '_' + args.model + '/' + log_dir + '.csv')
...
def main():
    ...
    if 'auto-attack' in adv_atc: 
      mean = (0.4914, 0.4822, 0.4465)
      std = (0.2023, 0.1994, 0.2010)
      model = nn.Sequential(Normalize(mean=mean, std=std), model)
    if adv_atc == 'auto-attack': 
      for eps in [2.0 / 255, 4.0 / 255]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'auto-attack', 'epsilon': eps, 'num_steps': 20, 'step_size': step_size, 
    ...
    if adv_atc == 'auto-attackL2': 
      for eps in [0.5, 1.0]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'auto-attackL2', 'epsilon': eps, 'num_steps': 20, 'step_size': step_size, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
        pgd_params['norm'] = 'L2'
    ...
    if adv_atc == 'pgd': 
      for eps in [2.0 / 255, 4.0 / 255, 8.0 / 255]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'pgd', 'epsilon': eps, 'num_steps': 20, 'step_size': step_size, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
    ...
    if adv_atc == 'pgdl2': 
      for eps in [3.0]:   
        pgd_params = {'attack': 'pgdl2', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1,
...
if __name__ == '__main__':
        ...
        for adv_atc in ['FW']:
            args.model = 'ResNet50'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/attack_cifar10/Madry-PGD-Linf/ResNet50/cifar_linf_8.pt'
            main()
        """ 
        # for adv_atc in ['FW', 'pgdl2']:
        # for adv_atc in ['pgd']:
        # for adv_atc in ['auto-attackL2']:
        # for adv_atc in ['FW', 'pgdl2', 'pgd', 'auto-attack', 'auto-attackL2']:
        for adv_atc in ['FW']:
            args.model = 'ResNet18'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/attack_cifar10/Madry-PGD-Linf/ResNet18/checkpoint.pt.best'
            main()
        """
        for adv_atc in ['FW']:
        # for adv_atc in ['FW', 'pgdl2']:
        ...
        # for adv_atc in ['auto-attackL2']:
            ...
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/attack_cifar10/Madry-PGD-Linf/WideResNet/checkpoint.pt.best'
            main()
        """
