...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Clean_Acc', 'Test_Loss', 'Test_Acc', 'Success_rate', 'Correct', 'Total', 'L2_Norm', 'Linf_Norm', 'L0_Norm', 'Lnuc_Norm'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            l2 = torch.norm(x_adv - x, p=2, dim=(1,2,3)).sum()
            l2norm += l2.item()
            linf = torch.norm(x_adv - x, p=float('inf'), dim=(1,2,3)).sum()
            ...
            with open(logname, 'a') as logfile:
               logwriter = csv.writer(logfile, delimiter=',')
               logwriter.writerow([batch_idx, 100.*correct/total, test_loss/(batch_idx+1), 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, correct_pgd, total, l2norm/total, linfnorm/total, eval_x_adv_l0/total, eval_x_adv_lnuc/total])
...
def mk_log_file(attack, num_steps, epsilon):
        ...
        if not os.path.isdir('loggerinf/log_rand_' + attack + '/'):
           os.mkdir('loggerinf/log_rand_' + attack + '/')
        if not os.path.isdir('summaryinf/sum_rand_' + attack + '/'):
           os.mkdir('summaryinf/sum_rand_' + attack + '/')

        logname = ('loggerinf/log_rand_' + attack + '/' + log_dir + '.csv')
        summaryname = ('summaryinf/sum_rand_' + attack + '/' + log_dir + '.csv')
...
def main():
    ...
    if adv_atc == 'FW':
      for eps in [0.5, 1.0, 1.5, 3.0]:  
        pgd_params = {'attack': 'FW', 'epsilon': eps, 'num_steps': 20, 'step_size': 1,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
    ...
    if adv_atc == 'pgd':
      for eps in [0.1, 0.2, 0.3]:
        pgd_params = {'attack': 'pgd', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1, 
    ...
    if adv_atc == 'auto-attack':
      for eps in [0.1, 0.2, 0.3]:
        pgd_params = {'attack': adv_atc, 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
...
if __name__ == '__main__':
        ...
        for adv_atc in ['pgd', 'pgdl2', 'FW', 'auto-attackL2', 'auto-attack']:
        # for adv_atc in ['pgd']:
        # for adv_atc in ['auto-attackL2', 'auto-attack']:
        # for adv_atc in ['FW']:
            args.model = 'SmallCNN'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/mnist_virginia/mnist/LeNet-inf2/mnist/f383270a-4119-4782-922a-a7a80444c226/checkpoint.pt.best'
            main()
        """
        ...
        for adv_atc in ['pgd', 'pgdl2', 'FW', 'auto-attackL2', 'auto-attack']:
            args.model = 'Net'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/mnist_virginia/mnist_Net/LeNet/mnist/36afc8e6-f18b-49d5-a5e8-b18cd7ce1be5/checkpoint.pt.best'
            main()
