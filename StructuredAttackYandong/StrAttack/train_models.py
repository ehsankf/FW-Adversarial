...
def main(args):
    ...
    if (not (args['adversarial'] or args['defensive'])):
        if args['dataset'] == "mnist" or args['dataset'] == "all":
            train(MNIST(), "models/mnist.h5", [32, 32, 64, 64, 200, 200], num_epochs=25)
        if args['dataset'] == 'cifar' or args['dataset'] == 'all':
            train(CIFAR(), "models/cifar.h5", [64, 64, 128, 128, 256, 256], num_epochs=25)

    if args['adversarial']:
        CWL2 = np.load('train/L2C_train.npy')
        ADMML2 = np.load('train/L2A_train.npy')
 #       ADMML0 = np.load('train/ADMML0_train.npy')

        labels = np.load('train/labels_train.npy')
        CWL2_ADMML2 = np.concatenate((CWL2, ADMML2), axis=0)
  #      CWL2_ADMML0 = np.concatenate((CWL2, ADMML0), axis=0)
        labels_2 = np.concatenate((labels, labels), axis=0)

        train(MNIST(), "models/mnist_cwl2", [32, 32, 64, 64, 200, 200], num_epochs=50,
              adversarial=True, examples=CWL2, labels=labels)
        train(MNIST(), "models/mnist_admml2", [32, 32, 64, 64, 200, 200], num_epochs=50,
              adversarial=True, examples=ADMML2, labels=labels)
   #     train(MNIST(), "models/mnist_admml0", [32, 32, 64, 64, 200, 200], num_epochs=50,
    #          adversarial=True, examples=ADMML0, labels=labels)
        train(MNIST(), "models/mnist_cwl2_admml2", [32, 32, 64, 64, 200, 200], num_epochs=50,
              adversarial=True, examples=CWL2_ADMML2, labels=labels_2)
  #      train(MNIST(), "models/mnist_cwl2_admml0", [32, 32, 64, 64, 200, 200], num_epochs=50,
     #         adversarial=True, examples=CWL2_ADMML0, labels=labels_2)
    ...
    if args['defensive']:
        if args['temp'] == 0:
            ...
            temp = args['temp']
    if args['defensive'] and (args['dataset'] == "mnist" or args['dataset'] == "mnist"):
        for t in temp:
            print('Mnist_'+str(t))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset", choices=["mnist", "cifar", "all"], default="cifar")
