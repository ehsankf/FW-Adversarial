...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            ind = ~predicted_pgd.eq(target).cpu().numpy()
            delta = (x_adv - x).detach().cpu().numpy()
            pdb.set_trace()
            if not ind.sum() == 0:
                IS_values += compute_IS_numpy(delta[ind], sali_map[ind].cpu().numpy()).sum()
                print("IS_values: ", IS_values)
            total += target.size(0)
...
def main(eps):
        ...
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False, 'batch_size': 9, 'maxiter': 100,
        'conf': 0, 'iteration_steps': 4, 'ro': 15, 'abort_early': True, 'retrain': False}
