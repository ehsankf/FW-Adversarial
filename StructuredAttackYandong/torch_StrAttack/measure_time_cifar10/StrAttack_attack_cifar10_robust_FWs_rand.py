...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
             mask, subsampling, size_groups, logname, summaryname, with_attack=True, norm='Linf', **kwargs):
        logging.info('eval')
        model.eval()
        eval_loss_clean = []
        eval_acc_clean = []
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Test_Loss', 'Clean_Test_Acc', 'Test_Acc', 'Success_rate', 
                              'Linf', 'L2_norm', 'L0_norm', 'Lnuc_norm', 'Correct', 'Total', 'Attack Time'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            x_adv = x_adv if (attack == 'CS' or attack == 'StrAttack' or 'auto-attack' in attack) else normalize(x_adv) 
            x = x if (attack == 'CS' or attack == 'StrAttack' or 'auto-attack' in attack) else normalize(x)
            ...
            with open(logname, 'a') as logfile:
               ...
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total,
                                  eval_x_adv_linf/succ_tot_a, eval_x_adv_l2/succ_tot_a, eval_x_adv_l0/succ_tot_a, eval_x_adv_lnuc/succ_tot_a, correct_pgd, total, attack_time])
...
def main():
    ...
    if adv_atc == 'FW_group' or adv_atc == 'FW' or adv_atc == 'fgsm' or adv_atc == 'pgd':
...
if __name__ == '__main__':
        ...
        # for adv_atc in ['FW', 'pgd', 'fgsm', 'FW_group', 'StrAttack', 'CS']:
        for adv_atc in ['auto-attack', 'auto-attackLinf']:

            args.model = 'ResNet50'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/attack_cifar10/Madry-PGD-L2/ResNet50/cifar_l2_0_5.pt'
            main()
        """
        for adv_atc in ['StrAttack']:
            args.model = 'ResNet18'
