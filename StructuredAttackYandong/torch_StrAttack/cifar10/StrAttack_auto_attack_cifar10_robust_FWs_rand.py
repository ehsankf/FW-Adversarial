...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
             mask, subsampling, size_groups, logname, summaryname, with_attack=True, norm='L2', **kwargs):
        logging.info('eval')
        model.eval()
        eval_loss_clean = []
        eval_acc_clean = []
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Test_Loss', 'Clean_Test_Acc', 'Test_Acc', 'Success_rate', 
                              'Linf', 'L2_norm', 'L0_norm', 'Lnuc_norm', 'IS_values', 'Correct', 'Total'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            if attack == 'pgd':
               ...
               x_adv = torch.from_numpy(x_adv).permute(0, 3, 1, 2).to(device)
                ...
                Strattack = LADMMSTL2(model, x.size(0), x.size(2), x.size(1), max_iterations=kwargs['maxiter'],
                               confidence=kwargs['conf'], binary_search_steps=kwargs['iteration_steps'], ro=kwargs['ro'],
                               abort_early=kwargs['abort_early'],retrain=kwargs['retrain']) 
            ...
            x_succ, x_adv_succ = x, x_adv    
            x_adv = x_adv if (attack == 'CS' or attack == 'StrAttack' or attack in ['auto-attack', 'auto-attackLinf']) else normalize(x_adv) 
            x = x if (attack == 'CS' or attack == 'StrAttack' or attack in ['auto-attack', 'auto-attackLinf']) else normalize(x)
            ...
            if not succ_num == 0:
                x_succ, x_adv_succ = x_succ[succ_ind], x_adv_succ[succ_ind]
                ...
                IS_values += compute_IS_numpy((x_adv_succ - x_succ).detach().cpu().numpy(), sali_map[succ_ind].cpu().numpy()).sum()
                succ_tot += succ_num
            ...
            with open(logname, 'a') as logfile:
               ...
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total,
                                  eval_x_adv_linf/succ_tot_a, eval_x_adv_l2/succ_tot_a, eval_x_adv_l0/succ_tot_a, eval_x_adv_lnuc/succ_tot_a, IS_values/succ_tot_a, correct_pgd, total])
...
def main():
    ...
    if adv_atc == 'auto-attack': 
      ...
      for eps in [0.5, 1.0, 1.5]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'auto-attack', 'epsilon': eps, 'norm': 'L2', 'num_steps': 20, 'step_size': step_size, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
    ...
    if adv_atc == 'auto-attackLinf': 
      ...
      for eps in [2./255., 4./255., 8./255.]:
        step_size =  2.0 / 255
        pgd_params = {'attack': 'auto-attackLinf', 'epsilon': eps, 'norm': 'L2', 'num_steps': 20, 'step_size': step_size, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
    ...
    if adv_atc == 'StrAttack': 
      for eps in [-1.0]:   
        pgd_params = {'attack': 'StrAttack', 'epsilon': eps, 'num_steps': 10, 'step_size': 3,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False, 'batch_size': 9, 'maxiter': 100,         'conf': 0, 'iteration_steps': 8, 'ro': 15, 'abort_early': True, 'retrain': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
        print('******************')
    ...
    if adv_atc == 'FW_group' or adv_atc == 'FW':
      ...
      for eps in [1.0, 3.0, 5.0]:  
        step_size =  1
        pgd_params = {'attack': adv_atc, 'epsilon': eps, 'num_steps': 20, 'step_size': step_size,
