...
sys.path.insert(0, '../../sparse_imperceivable_attacks/')
import cornersearch_attacks_pt

sys.path.insert(0, '/data/ehsan/Adv-Ex/test_attack2/test-attackYan')
...
def compute_IS_numpy(delta, sali_map, perc=30):
        ...
        nu = np.percentile(ASM, perc, axis=(0))
...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
             mask, subsampling, size_groups, logname, summaryname, with_attack=True, norm='L2', **args):
        logging.info('eval')
        model.eval()
        eval_loss_clean = []
        eval_acc_clean = []
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Clean_Acc', 'Test_Loss', 'Test_Acc', 'Success_rate', 'Correct', 'Total', 'L2_Norm', 'Linf_Norm', 'L0_Norm', 'Lnuc_Norm', 'IS_values'])
        ...
        X, X_ADV, SAL_MAT, Y, Y_ADV, ADV_IND = [], [], [], [], [], []
        detach = lambda x : x.cpu().detach() 
        cat = lambda x : torch.cat(x, 0)

        if attack == 'auto-attack' or attack == 'auto-attackLinf':
            adversary = AutoAttack(model, norm=norm, eps=epsilon, log_path='log.tex')
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            if attack == 'pgd':
               ...
               x_adv.to(device)
                ...
                x = x[:]
                target = target[:]
                one_hot_targets = Identity[target.data.cpu()]
            ...
            elif attack == 'auto-attack' or attack == 'auto-attackLinf':
            ...
            sali_map = compute_salien_map(model, x, predicted_pgd)

            X.append(detach(x)), X_ADV.append(detach(x_adv)), SAL_MAT.append(detach(sali_map)), Y.append(detach(predicted)), Y_ADV.append(detach(predicted_pgd)), ADV_IND.append(detach(succ_ind)) 

            if not succ_num == 0: 
                x_adv_suc = x_adv[succ_ind].detach()
                x_suc = x[succ_ind].detach()
                l2norm += torch.norm(x_adv_suc - x_suc, p=2, dim=(1,2,3)).sum().item()
                linfnorm += torch.norm(x_adv_suc - x_suc, p=float('inf'), dim=(1,2,3)).sum().item()
                eval_x_adv_l0 += torch.sum(torch.max(torch.abs(x_suc - x_adv_suc) > 1e-10, dim=1)[0], dim=(1,2)).sum().item()
                eval_x_adv_lnuc += torch.norm(x_suc[:, 0, :, :] - x_adv_suc[:, 0, :, :], p='nuc', dim = (1,2)).sum().item()
                IS_values += compute_IS_numpy((x_adv_suc - x_suc).detach().cpu().numpy(), sali_map[succ_ind].cpu().numpy()).sum()
                succ_tot += succ_num
                
   
            succ_tot_a = succ_tot if not succ_tot == 0 else 1.
            ...
            with open(logname, 'a') as logfile:
               logwriter = csv.writer(logfile, delimiter=',')
               logwriter.writerow([batch_idx, 100.*correct/total, test_loss/(batch_idx+1), 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, correct_pgd, total, l2norm/succ_tot_a, linfnorm/succ_tot_a, eval_x_adv_l0/succ_tot_a, eval_x_adv_lnuc/succ_tot_a, IS_values/succ_tot_a])

        X, X_ADV, SAL_MAT, Y, Y_ADV, ADV_IND = cat(X), cat(X_ADV), cat(SAL_MAT), cat(Y), cat(Y_ADV), cat(ADV_IND)    
        IS_values = [compute_IS_numpy((X_ADV[ADV_IND] - X[ADV_IND]).detach().cpu().numpy(), \
                      SAL_MAT[ADV_IND].cpu().numpy(), nu).sum() \
                      for nu in range(30, 100, 10)]
        IS_values = np.divide(IS_values, len(ADV_IND[ADV_IND]))
        torch.save({'X': X, 'X_ADV': X_ADV, 'SAL_MAT': SAL_MAT, 'Y': Y, 'Y_ADV': Y_ADV, 'IS_values': IS_values}, f'{summaryname}_dict.pt')
        load_file = torch.load(f'{summaryname}_dict.pt')
        '''    
...
def main():
    ...
    if adv_atc == 'auto-attack': 
      for eps in [0.5, 1.0, 1.5]:   
        pgd_params = {'attack': 'auto-attack', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1,
    ...
    if adv_atc == 'auto-attackLinf': 
      for eps in [0.1, 0.2, 0.3]:   
        pgd_params = {'attack': 'auto-attackLinf', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        pgd_params['norm'] = 'Linf'
        run_eval(model, **pgd_params)
        print('******************')

    if adv_atc == 'CS': 
      for eps in [-1.0]:   
        pgd_params = {'attack': adv_atc, 'epsilon': eps, 'num_steps': 10, 'step_size': 3,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False, 'batch_size': 9, 'maxiter': 100,         'conf': 0, 'iteration_steps': 4, 'ro': 15, 'abort_early': True, 'retrain': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
        print('******************')
    if adv_atc == 'StrAttack': 
      for eps in [-1.0]:   
        pgd_params = {'attack': adv_atc, 'epsilon': eps, 'num_steps': 10, 'step_size': 3,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False, 'batch_size': 9, 'maxiter': 1000,         'conf': 0, 'iteration_steps': 8, 'ro': 15, 'abort_early': True, 'retrain': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
        print('******************')

    if adv_atc == 'FW_group' or adv_atc == 'FW':
      for eps in [1.0, 3.0, 5.0]:  
        pgd_params = {'attack': adv_atc, 'epsilon': eps, 'num_steps': 20, 'step_size': 1,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
...
if __name__ == '__main__':
        ...
        testset = torchvision.datasets.MNIST(root='/data/ehsan/Adv-Ex/test_attack2/data', train=False, download=True, transform=transform_test)
        ...
        # for adv_atc in ['StrAttack', 'CS']:
        # for adv_atc in ['auto-attack']:
        for adv_atc in ['auto-attackLinf']:
            args.model = 'SmallCNN'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/mnist_virginia/mnist/LeNet-L2/mnist/68f8b596-a27f-4d61-9903-44f1b33a8b97/checkpoint.pt.best'
            main()


        # for adv_atc in ['FW', 'FW_group', 'StrAttack', 'CS']:
        for adv_atc in ['auto-attackLinf']:
            args.model = 'Net'
            args.model_path = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/mnist_virginia/mnist_Net/LeNet-L2/mnist/7a6b69a8-46f7-4742-9734-e3eee15a999d/checkpoint.pt.best'
            main()
