...
def mk_log_file(attack, num_steps, epsilon):
        ...
        log_dir = attack + 'iter' + str(num_steps) + 'eps' + str(epsilon) + args.model + args.model_path.split('/')[0].split('.')[0]
...
def main():
    ...
    if adv_atc == 'auto-attackLinfinf': 
      for eps in [0.1, 0.2, 0.3]:   
        pgd_params = {'attack': 'auto-attackLinfinf', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': False, 'size_groups': False}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("%s parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['attack'], pgd_params['epsilon'], pgd_params['num_steps']))
    ...
    if adv_atc == 'pgd':
      for eps in [0.1, 0.2, 0.3]:
        pgd_params = {'attack': 'pgd', 'epsilon': eps, 'num_steps': 7, 'step_size': 0.1,
...
if __name__ == '__main__':
        ...
        for adv_atc in ['auto-attackLinfinf']:
            args.model = 'Net'
            args.model_path = 'net_robust_adversarial_eps5_FW/net_epoch190.ckpt'
            main()
        ...
        for adv_atc in ['auto-attack', 'auto-attackLinf']:
            args.model = 'Net'
            args.model_path = 'net_robust_adversarial_eps3_FW/net_adver_game_epoch190.ckpt'
            main()
        """
