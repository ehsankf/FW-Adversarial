...
from saliency_adversarial_example import saliency_map_mat
import sparse_imperceivable_attacks.cornersearch_attacks_pt as cornersearch_attacks_pt
...
def run_eval(model, attack, num_steps, step_size, p, type_ball,
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            if attack == 'pgd':
               ...
               x_adv.to(device) 
                ...
                x = x[:]
                target = target[:]
                one_hot_targets = Identity[target.data]
            ...
            elif attack == 'SparseAttack':
                SparseAttack = cornersearch_attacks_pt.CSattack(model, args)
                x_adv, pixels_changed, fl_success = SparseAttack.perturb(x.permute(0, 2, 3, 1).detach().cpu().numpy(), y.detach().cpu().numpy())
                x_adv = torch.from_numpy(x_adv).float()
                x_adv = x_adv.permute(0, 3, 1, 2).to(device)
            ...
            linf_norm = torch.norm(x_adv-x, p=float('inf')).mean()
            ...
            print(batch_idx, len(test_loader), 'Loss: %.3f | Linf: %.3f | Acc: %.3f%% Successs: %.3f%% (%d/%d)' 
            % (test_loss/(batch_idx+1), linf_norm, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, correct_pgd, total))
...
def main(eps):
        ...
        pgd_params = {'attack': 'SparseAttack', 'epsilon': eps, 'num_steps': 10, 'step_size': 3,
        ...
        """ sparse_attack_args = {'type_attack': 'L0',
            'n_iter': 1000,
            'n_max': 100,
            'kappa': -1,
            'epsilon': 0.1,
        ...
        sparse_attack_args = {'type_attack': 'L0+Linf',
            'n_iter': 100,
            'n_max': 150,
            'kappa': 0.8,
            'epsilon': 0.1,
        ...
        pgd_params.update(sparse_attack_args)
