...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Clean_Acc', 'Test_Loss', 'Test_Acc', 'Success_rate', 'Correct', 'Total', 'L2_Norm', 'Linf_Norm', 'L0_Norm', 'Lnuc_Norm', 'SSIM', 'PSNR'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            eval_mse += calculate_mse(x.detach().cpu().numpy(), x_adv.detach().cpu().numpy())
            ...
            with open(logname, 'a') as logfile:
               logwriter = csv.writer(logfile, delimiter=',')
               logwriter.writerow([batch_idx, 100.*correct/total, test_loss/(batch_idx+1), 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, correct_pgd, total, l2norm/total, linfnorm/total, eval_x_adv_l0/total, eval_x_adv_lnuc/total, eval_ssim, eval_psnr])
...
def main():
    ...
    if adv_atc == 'FW' or adv_atc == 'FW_group':
    ...
    if adv_atc == 'fgsm':
      for eps in [0.1, 0.2, 0.3]:
        pgd_params = {'attack': 'fgsm', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1, 
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
        run_eval(model, **pgd_params)
...
if __name__ == '__main__':
        ...
        for adv_atc in ['FW_group']:
        # for adv_atc in ['pgd']:
        # for adv_atc in ['fgsm', 'pgd', 'pgdl2', 'FW']:
            args.model = 'SmallCNN'
