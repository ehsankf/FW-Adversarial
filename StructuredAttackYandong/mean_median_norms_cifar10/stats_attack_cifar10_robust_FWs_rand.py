...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Test_Loss', 'Clean_Test_Acc', 'Test_Acc', 'Success_rate', 
                              'Linf_mean', 'Linf_median', 'L2_mean', 'L2_median', 'L0_mean', 'L0_median', 'Lnuc_mean', 'Luc_median', 'Total'])
        ...
        cat = lambda x : torch.cat(x, 0).float()
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            eval_x_adv_linf = torch.norm(adv_perturb, p = float("inf"), dim = 1).data
            eval_x_adv_l0 = torch.sum(torch.max(torch.abs(x_succ - x_adv_succ) > 1e-10, dim=1)[0], dim=(1,2)).data
            eval_x_adv_l2 = torch.norm(adv_perturb, p=2, dim = 1).data    
            eval_x_adv_lnuc = torch.norm(x_succ[:, 0, :, :] - x_adv_succ[:, 0, :, :], p='nuc', dim = (1,2)).data

            LINF.append(detach(eval_x_adv_linf)), L2.append(detach(eval_x_adv_l2)), L0.append(detach(eval_x_adv_l0)), LNUC.append(detach(eval_x_adv_lnuc))
            ...
            % (eval_x_adv_linf.sum()/succ_tot_a, eval_x_adv_l2.sum()/succ_tot_a, eval_x_adv_l0.sum()/succ_tot_a, eval_x_adv_lnuc.sum()/succ_tot_a, total))
            

        LINF, L2, L0, LNUC = cat(LINF), cat(L2), cat(L0), cat(LNUC)   
        with open(logname, 'a') as logfile:
               logwriter = csv.writer(logfile, delimiter=',')
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total, torch.mean(LINF).item(), torch.median(LINF).item(), torch.mean(L2).item(), torch.median(L2).item(), torch.mean(L0).item(), torch.median(L0).item(), torch.mean(LNUC).item(), torch.median(LNUC).item(), total])
...
def main(eps):
     ...
     base_dir = '/data/ehsan/Adv-Ex/test_attack2/test-attackYan/attack_cifar10/'
     args.model_path = base_dir + args.model_path
     model = load_model()
