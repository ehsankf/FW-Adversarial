...
sys.path.insert(0, '../../../../test_attack2/test-attackYan/attack_cifar10/')
...
def run_eval(model, attack, epsilon, num_steps, step_size, p, type_ball,
        ...
        with open(logname, 'w') as logfile:
           ...
           logwriter.writerow(['batch_idx', 'Test_Loss', 'Clean_Test_Acc', 'Test_Acc', 'Success_rate', 
                              'Linf', 'L2_norm', 'L0_norm', 'Lnuc_norm', 'Correct', 'Total', 'SSIM'])
        ...
        for batch_idx, (data, target) in enumerate(tqdm(test_loader)):
            ...
            if not succ_num == 0:
                ...
                metric.update([x_adv_succ, x_succ])
            ...
            with open(logname, 'a') as logfile:
               ...
               logwriter.writerow([batch_idx, test_loss/(batch_idx+1), 100.*correct/total, 100.*correct_pgd/total, 100.*(total - correct_pgd)/total,
                                  eval_x_adv_linf/succ_tot_a, eval_x_adv_l2/succ_tot_a, eval_x_adv_l0/succ_tot_a, eval_x_adv_lnuc/succ_tot_a, correct_pgd, total, eval_ssim])
...
def mk_log_file(attack, num_steps, epsilon):
        ...
        if not os.path.isdir('summary_1000'):
            os.mkdir('summary_1000')

        if not os.path.isdir('logger_1000'):
            os.mkdir('logger_1000')

        log_dir = attack + 'iter' + str(num_steps) + 'eps' + str(epsilon) + args.model
        if not os.path.isdir('logger_1000/log_rand_cross_' + attack + '_' + args.model + '/'):
           os.mkdir('logger_1000/log_rand_cross_' + attack + '_' + args.model + '/')
        if not os.path.isdir('summary_1000/sum_rand_cross_' + attack + '_' + args.model + '/'):
           os.mkdir('summary_1000/sum_rand_cross_' + attack + '_' + args.model + '/')

        logname = ('logger_1000/log_rand_cross_' + attack + '_' + args.model + '/' + log_dir + '.csv')
        summaryname = ('summary_1000/sum_rand_cross_' + attack + '_' + args.model + '/' + log_dir + '.csv')
...
def main():
    ...
    if adv_atc == 'auto-attack':
      ...
      for eps in [4.0 / 255, 8.0 / 255, 16. /255., 32. / 255. ]:
    ...
    if adv_atc == 'auto-attackL2':
      for eps in [1.0, 2.0, 3.0]:
        pgd_params = {'attack': 'auto-attackL2', 'epsilon': eps, 'num_steps': 20, 'step_size': 0.1,
        'p': args.p, 'type_ball': 'nuclear', 'mask': args.mask, 'subsampling': args.subsampling, 'size_groups': args.size_groups}
        pgd_params['summaryname'], pgd_params['logname'] = mk_log_file(pgd_params['attack'], pgd_params['num_steps'], pgd_params['epsilon'])
        print("pgd parameters... epsilon: %.3f%% number of steps: %d"% (pgd_params['epsilon'], pgd_params['num_steps']))
        pgd_params['norm'] = 'L2'
    ...
    if adv_atc == 'pgdl2': 
      for eps in [1.0, 1.5, 2.0, 3.0, 5.0]: 
        pgd_params = {'attack': 'pgdl2', 'epsilon': eps, 'num_steps': 10, 'step_size': 0.1,
...
if __name__ == '__main__':
        ...
        # for adv_atc in ['pgdl2', 'pgd']:
        for adv_atc in ['auto-attackL2', 'pgdl2', 'pgd', 'FW']:
        # for adv_atc in ['auto-attack']:

            args.model = 'resnet50'
            args.model_path = 'imagenet_linf_8.pt' # 'imagenet_l2_3_0.pt'
